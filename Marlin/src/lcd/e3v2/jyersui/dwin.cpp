      ...
      GRID_LOOP(x, y) {
        ...
        if (viewer_print_value) {
          ...
          if (isnan(bedlevel.z_values[x][y])) {  // undefined
            DWIN_Draw_String(false, font6x12, Def_Text_Color, Color_Bg_Blue, start_x_px + cell_width_px / 2 - 5, start_y_px + offset_y, F("X"));
          }
          else {                          // has value
            ...
            if (!(GRID_MAX_POINTS_X < 10))
              DWIN_Draw_String(false, font6x12, Def_Text_Color, Color_Bg_Blue, start_x_px - 2 + offset_x, start_y_px + offset_y /*+ square / 2 - 6*/, F("."));
            DWIN_Draw_String(false, font6x12, Def_Text_Color, Color_Bg_Blue, start_x_px + 1 + offset_x, start_y_px + offset_y /*+ square / 2 - 6*/, buf);
          }
          safe_delay(10);
          LCD_SERIAL.flushTX();
        }
...
void CrealityDWINClass::Draw_Float(float value, uint8_t row, bool selected/*=false*/, uint8_t minunit/*=10*/) {
  ...
  if (isnan(value))
    DWIN_Draw_String(true, DWIN_FONT_MENU, Def_Text_Color, bColor, xpos - 8, MBASE(row), F(" NaN"));
  else {
    ...
    DWIN_Draw_String(true, DWIN_FONT_MENU, Def_Text_Color, bColor, xpos - 8, MBASE(row), value < 0 ? F("-") : F(" "));
  }
}
...
void CrealityDWINClass::Draw_Option(uint8_t value, const char * const * options, uint8_t row, bool selected/*=false*/, bool color/*=false*/) {
  ...
  void CrealityDWINClass::Draw_String(char * string, uint8_t row, bool selected/*=false*/, bool below/*=false*/) {
    ...
    DWIN_Draw_String(true, DWIN_FONT_MENU, Def_Text_Color, (selected) ? Def_Selected_Color : Def_Background_Color, offset_x, MBASE(row)-1+offset_y, string);
  }
...
void CrealityDWINClass::Draw_Title(const char * ctitle) {
  DWIN_Draw_String(false, DWIN_FONT_HEAD, GetColor(eeprom_settings.menu_top_txt, Def_TitleTxt_color, false), Def_TitleBg_color, (DWIN_WIDTH - strlen(ctitle) * STAT_CHR_W) / 2, 5, ctitle);
}
void CrealityDWINClass::Draw_Title(FSTR_P const ftitle) {
  DWIN_Draw_String(false, DWIN_FONT_HEAD, GetColor(eeprom_settings.menu_top_txt, Def_TitleTxt_color, false), Def_TitleBg_color, (DWIN_WIDTH - strlen_P(FTOP(ftitle)) * STAT_CHR_W) / 2, 5, ftitle);
}
...
void _Decorate_Menu_Item(uint8_t row, uint8_t icon, bool more) {
  ...
  DWIN_Draw_Line(CrealityDWIN.GetColor(eeprom_settings.menu_split_line, Def_SplitLine_Color, true), 16, MBASE(row) + 33, 256, MBASE(row) + 33); // Draw Menu Line
}
...
void CrealityDWINClass::Draw_Menu_Item(uint8_t row, uint8_t icon/*=0*/, const char * label1, const char * label2, bool more/*=false*/, bool centered/*=false*/) {
  ...
  if (label1) DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, label1_offset_x, MBASE(row) - 1 - label_offset_y, label1); // Draw Label
  if (label2) DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, label2_offset_x, MBASE(row) - 1 + label_offset_y, label2); // Draw Label
...
void CrealityDWINClass::Draw_Menu_Item(uint8_t row, uint8_t icon/*=0*/, FSTR_P const flabel1, FSTR_P const flabel2, bool more/*=false*/, bool centered/*=false*/) {
  ...
  if (flabel1) DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, label1_offset_x, MBASE(row) - 1 - label_offset_y, flabel1); // Draw Label
  if (flabel2) DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, label2_offset_x, MBASE(row) - 1 + label_offset_y, flabel2); // Draw Label
  _Decorate_Menu_Item(row, icon, more);
}
...
void CrealityDWINClass::Main_Menu_Icons() {
  ...
  else DWINUI::DRAW_IconWB(ICON, ICON_Print_0, 17, 110);
  ...
  else DWINUI::DRAW_IconWB(ICON, ICON_Prepare_0, 145, 110);
  ...
  else DWINUI::DRAW_IconWB(ICON, ICON_Control_0, 17, 226);
  DWINUI::Draw_String(43, 297, GET_TEXT_F(MSG_CONTROL));
    ...
    else DWINUI::DRAW_IconWB(ICON, ICON_Leveling_0, 145, 226);
    ...
    else DWINUI::DRAW_IconWB(ICON, ICON_Info_0, 145, 226);
...
void CrealityDWINClass::Print_Screen_Icons() {
  ...
  else DWINUI::DRAW_IconWB(ICON, ICON_Setup_0, 8, 252);
  ...
  else DWINUI::DRAW_IconWB(ICON, ICON_Stop_0, 184, 252);
  ...
  if (temp_val.paused) {
    ...
    else DWINUI::DRAW_IconWB(ICON, ICON_Continue_0, 96, 252);
    ...
    else DWINUI::DRAW_IconWB(ICON, ICON_Pause_0, 96, 252);
...
void CrealityDWINClass::Draw_Print_Screen() {
  ...
  DWIN_Draw_String(false, DWIN_FONT_MENU, Def_PercentTxt_Color, Def_Background_Color, 41, 163, GET_TEXT_F(MSG_INFO_PRINT_TIME));
  DWIN_Draw_String(false, DWIN_FONT_MENU, Def_PercentTxt_Color, Def_Background_Color, 176, 163, GET_TEXT_F(MSG_REMAINING_TIME));
  Update_Status_Bar(true);
...
void CrealityDWINClass::Draw_Print_Filename(const bool reset/*=false*/) {
  ...
  if (process == Print) {
    ...
    if (pos > STATUS_CHAR_LIMIT) {
      ...
      DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, npos, 60, dispname);
      ...
      namescrl++;
        ...
        DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, npos, 60, filename);
    }
  }
}
...
void CrealityDWINClass::Draw_Print_ProgressBar() {
  ...
  DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_percent, Def_PercentTxt_Color), Def_Background_Color, 134, 133, F("%"));
}
  ...
  void CrealityDWINClass::Draw_Print_ProgressRemain() {
    ...
    if (eeprom_settings.time_format_textual) {
      DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_time, Def_PercentTxt_Color), Def_Background_Color, 193, 187, F("h"));
      DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_time, Def_PercentTxt_Color), Def_Background_Color, 217, 187, F("m"));
    }
    else
      DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_time, Def_PercentTxt_Color), Def_Background_Color, 193, 187, F(":"));
  }
...
void CrealityDWINClass::Draw_Print_ProgressElapsed() {
  ...
  if (eeprom_settings.time_format_textual) {
    DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_time, Def_PercentTxt_Color), Def_Background_Color, 59, 187, F("h"));
    DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_time, Def_PercentTxt_Color), Def_Background_Color, 83, 187, F("m"));
  }
  else
    DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.progress_time, Def_PercentTxt_Color), Def_Background_Color, 59, 187, F(":"));
}
...
void CrealityDWINClass::Draw_SD_List(bool removed/*=false*/) {
  ...
  if (card.isMounted() && !removed) {
    ...
    DWIN_Draw_String(false, font16x32, Def_AlertTxt_Color, Def_AlertBg_Color, ((DWIN_WIDTH) - 8 * 16) / 2, MBASE(3), GET_TEXT_F(MSG_NO_MEDIA));
  }
...
void CrealityDWINClass::Draw_Status_Area(bool icons/*=false*/) {
  ...
  if (icons) DWIN_Draw_Rectangle(1, Def_Background_Color, 0, STATUS_Y, DWIN_WIDTH, DWIN_HEIGHT - 1);
    ...
    if (icons) {
      ...
      DWIN_Draw_String(false, DWIN_FONT_STAT, GetColor(eeprom_settings.status_area_text, Def_Indicator_Color), Def_Background_Color, 25 + 3 * STAT_CHR_W + 5, 384, F("/"));
    }
    ...
    if (icons) {
      ...
      DWIN_Draw_String(false, DWIN_FONT_STAT, GetColor(eeprom_settings.status_area_text, Def_Indicator_Color), Def_Background_Color, 116 + 5 * STAT_CHR_W + 2, 417, F("%"));
    }
    ...
    if (icons) {
      ...
      DWIN_Draw_String(false, DWIN_FONT_STAT, GetColor(eeprom_settings.status_area_text, Def_Indicator_Color), Def_Background_Color, 25 + 3 * STAT_CHR_W + 5, 417, F("/"));
    }
  ...
  if (icons) {
    ...
    DWIN_Draw_String(false, DWIN_FONT_STAT, GetColor(eeprom_settings.status_area_text, Def_Indicator_Color), Def_Background_Color, 116 + 5 * STAT_CHR_W + 2, 384, F("%"));
  }
  ...
  if (update_x) {
    ...
    if ((update_x = axis_should_home(X_AXIS) && ui.get_blink()))
      DWIN_Draw_String(true, DWIN_FONT_MENU, GetColor(eeprom_settings.coordinates_text, Def_Coordinate_Color), Def_Background_Color, 39, 459, F("  -?-  "));
    else
  ...
  if (update_y) {
    ...
    if ((update_y = axis_should_home(Y_AXIS) && ui.get_blink()))
      DWIN_Draw_String(true, DWIN_FONT_MENU, GetColor(eeprom_settings.coordinates_text, Def_Coordinate_Color), Def_Background_Color, 124, 459, F("  -?-  "));
    else
  ...
  if (update_z) {
    ...
    if ((update_z = axis_should_home(Z_AXIS) && ui.get_blink()))
      DWIN_Draw_String(true, DWIN_FONT_MENU, GetColor(eeprom_settings.coordinates_text, Def_Coordinate_Color), Def_Background_Color, 205, 459, F("  -?-  "));
    else
...
void CrealityDWINClass::Draw_Popup(FSTR_P const line1, FSTR_P const line2, FSTR_P const line3, uint8_t mode, uint8_t icon/*=0*/) {
  ...
  DWIN_Draw_String(true, DWIN_FONT_MENU, Def_PopupTxt_Color, Def_PopupBg_color, (272 - 8 * strlen_P(FTOP(line1))) / 2, ypos, line1);
  DWIN_Draw_String(true, DWIN_FONT_MENU, Def_PopupTxt_Color, Def_PopupBg_color, (272 - 8 * strlen_P(FTOP(line2))) / 2, ypos + 30, line2);
  DWIN_Draw_String(true, DWIN_FONT_MENU, Def_PopupTxt_Color, Def_PopupBg_color, (272 - 8 * strlen_P(FTOP(line3))) / 2, ypos + 60, line3);
  if (mode == Popup) {
    selection = 0;
...
void CrealityDWINClass::Update_Status_Bar(bool refresh/*=false*/) {
  ...
  if (pos > STATUS_CHAR_LIMIT) {
    pos -= msgscrl;
    len = _MIN(pos, STATUS_CHAR_LIMIT);
    char dispmsg[len + 1];
    if (pos >= 0) {
    ...
    if (process == Print) {
      ...
      const int8_t npos = (DWIN_WIDTH - STATUS_CHAR_LIMIT * MENU_CHR_W) / 2;
      DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.status_bar_text, Def_StatusTxt_Color), Def_StatusBg_Color, npos, 219, dispmsg);
    }
    else {
      DWIN_Draw_Rectangle(1, Def_StatusBg_Color, 8, 352, DWIN_WIDTH - 8, 376);
      const int8_t npos = (DWIN_WIDTH - STATUS_CHAR_LIMIT * MENU_CHR_W) / 2;
      DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.status_bar_text, Def_StatusTxt_Color), Def_StatusBg_Color, npos, 357, dispmsg);
    }
    ...
    if (new_msg) {
      ...
      if (process == Print) {
        ...
        DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.status_bar_text, Def_StatusTxt_Color), Def_StatusBg_Color, npos, 219, statusmsg);
      }
      else {
        DWIN_Draw_Rectangle(1, Def_StatusBg_Color, 8, 352, DWIN_WIDTH - 8, 376);
        const int8_t npos = (DWIN_WIDTH - strlen(statusmsg) * MENU_CHR_W) / 2;
        DWIN_Draw_String(false, DWIN_FONT_MENU, GetColor(eeprom_settings.status_bar_text, Def_StatusTxt_Color), Def_StatusBg_Color, npos, 357, statusmsg);
      }
    }
  }
}
  ...
  void CrealityDWINClass::Draw_Keys(uint8_t index, bool selected, bool uppercase/*=false*/, bool lock/*=false*/) {
    ...
    if (index < 28) {
      if (index == 19) {
        ...
        DWIN_Draw_String(true, DWIN_FONT_MENU, Color_Red, Def_Background_Color, KEY_X1(7)+xOffset[2]+3, KEY_Y1(2)+5, F("<--"));
      }
      else {
        if (index > 19) index--;
        if (index > 27) index--;
        ...
        DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(x)+xOffset[y]+5, KEY_Y1(y)+5, keyStr);
      ...
      switch (index) {
        case 28:
          ...
          DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(0)-1, KEY_Y1(3)+5, F("?123"));
          break;
        case 29:
          ...
          DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(1)+xOffset[1]+5, KEY_Y1(3)+5, F("-"));
          break;
        case 30:
          ...
          DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(2)+xOffset[1]+5, KEY_Y1(3)+5, F("_"));
          break;
        case 31:
          ...
          DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(3)+xOffset[1]+14, KEY_Y1(3)+5, F("Space"));
        ...
        case 32:
          ...
          DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(6)+xOffset[1]+7, KEY_Y1(3)+5, F("."));
          if (keyboard_restrict) {
        ...
        case 33:
          ...
          DWIN_Draw_String(false, DWIN_FONT_MENU, Def_Text_Color, Def_Background_Color, KEY_X1(7)+xOffset[1]+4, KEY_Y1(3)+5, F("/"));
          if (keyboard_restrict) {
        ...
        case 34:
          ...
          DWIN_Draw_String(true, DWIN_FONT_MENU, Color_Cyan, Def_Background_Color, KEY_X1(7)+xOffset[2]+3, KEY_Y1(3)+5, F("-->"));
          break;
      }
    }
  }
...
void CrealityDWINClass::Menu_Item_Handler(uint8_t menu, uint8_t item, bool draw/*=true*/) {
  ...
  switch (menu) {
      ...
      switch (item) {
        ...
        case CONTROL_VISUAL:
          if (draw)
            ...
            Draw_Menu(Visual);
          break;
        #if ENABLED(HOST_ACTION_COMMANDS, HAS_HOSTACTION_MENUS) && DISABLED(HAS_RC_CPU_AND_UBL)
    ...
    case Motion:
      ...
      #define MOTION_INVERT_E0 (MOTION_STEPS + (ENABLED(HAS_HOTEND, JYENHANCED)))
      #define MOTION_FLOW (MOTION_INVERT_E0 + ENABLED(HAS_HOTEND))
      ...
      switch (item) {
        ...
        #if HAS_HOTEND
          #if JYENHANCED
            case MOTION_INVERT_E0:
              if (draw) {
                Draw_Menu_Item(row, ICON_Motion, F("Invert Extruder"));
    ...
    #if BOTH(NOZZLE_PARK_FEATURE, JYENHANCED)
      case Parkmenu:
        ...
        switch (item) {
          ...
          case PARKMENU_POSZ:
            if (draw) {
              Draw_Menu_Item(row, ICON_ParkPosZ, F("Park Z Raise"));
    ...
    case Advanced:
      ...
      #define ADVANCED_PHYSICAL (ADVANCED_BEEPER + ENABLED(JYENHANCED))
      ...
      switch (item) {
        case ADVANCED_BACK:
          ...
          case ADVANCED_PHYSICAL:
            if (draw)
              Draw_Menu_Item(row, ICON_PhySet, F("Physical Settings"), nullptr, true);
            else {
              Update_Status("Warning! increase X/Y max position before increase bed size.");
      ...
      case PhySetMenu:
        ...
        switch (item) {
          ...
          case PHYSET_XBEDSIZE:
            if (draw) {
              Draw_Menu_Item(row, ICON_BedSizeX, F("X bed size"));
              Draw_Float(eeprom_settings.x_bed_size, row, false, 1);
            }
            else {
              temp_val.axis = NO_AXIS_ENUM;
              Modify_Value(eeprom_settings.x_bed_size, X_BED_MIN, X_MAX_POS, 1);
            }
            break;
          case PHYSET_YBEDSIZE:
            if (draw) {
              Draw_Menu_Item(row, ICON_BedSizeY, F("Y bed size"));
              Draw_Float(eeprom_settings.y_bed_size, row, false, 1);
            }
            else {
              temp_val.axis = NO_AXIS_ENUM;
              Modify_Value(eeprom_settings.y_bed_size, Y_BED_MIN, Y_MAX_POS, 1);
            }
            break;
          case PHYSET_XMINPOS:
            if (draw) {
              Draw_Menu_Item(row, ICON_MaxPosX, F("X min position"));
              Draw_Float(eeprom_settings.x_min_pos, row, false, 1);
            }
            else {
              temp_val.axis = X_AXIS;
              Modify_Value(eeprom_settings.x_min_pos, -500, 500, 1);
            }
            break;
          case PHYSET_YMINPOS:
            if (draw) {
              Draw_Menu_Item(row, ICON_MaxPosY, F("Y min position"));
              Draw_Float(eeprom_settings.y_min_pos, row, false, 1);
            }
            else {
              temp_val.axis = Y_AXIS;
              Modify_Value(eeprom_settings.y_min_pos, -500, 500, 1);
            }
            break;
          case PHYSET_XMAXPOS:
            if (draw) {
              Draw_Menu_Item(row, ICON_MaxPosX, F("X max position"));
              Draw_Float(eeprom_settings.x_max_pos, row, false, 1);
            }
            else {
              TERN_(NOZZLE_PARK_FEATURE, temp_val.last_pos = eeprom_settings.x_max_pos);
              temp_val.axis = X_AXIS;
              Modify_Value(eeprom_settings.x_max_pos, X_BED_MIN, 999, 1);
            }
            break;
          case PHYSET_YMAXPOS:
            if (draw) {
              Draw_Menu_Item(row, ICON_MaxPosY, F("Y max position"));
              Draw_Float(eeprom_settings.y_max_pos, row, false, 1);
            }
            else {
              TERN_(NOZZLE_PARK_FEATURE, temp_val.last_pos = eeprom_settings.y_max_pos);
              temp_val.axis = Y_AXIS;
              Modify_Value(eeprom_settings.y_max_pos, Y_BED_MIN, 999, 1);
            }
            break;
          case PHYSET_ZMAXPOS:
            if (draw) {
              Draw_Menu_Item(row, ICON_MaxPosZ, F("Z max position"));
              Draw_Float(eeprom_settings.z_max_pos, row, false, 1);
            }
            else {
              temp_val.axis = Z_AXIS;
              Modify_Value(eeprom_settings.z_max_pos, 100, 999, 1);
            }
            break;
        }
      break;
      ...
      case ProbeMenu:
        ...
        #define PROBE_PMARGIN (PROBE_ZOFFSET + ENABLED(JYENHANCED))
        #define PROBE_Z_FEEDR_FAST (PROBE_PMARGIN + ENABLED(JYENHANCED))
        #define PROBE_Z_FEEDR_SLOW (PROBE_Z_FEEDR_FAST + ENABLED(JYENHANCED))
          ...
          case PROBE_ZOFFSET:
            ...
            case PROBE_PMARGIN:
              if (draw) {
                ...
                Draw_Float(eeprom_settings.probing_margin, row, false, 1);
                }
              else
                Modify_Value(eeprom_settings.probing_margin, MIN_PROBE_MARGIN, MAX_PROBE_MARGIN, 1);
              break;
            case PROBE_Z_FEEDR_FAST:
              if (draw) {
                Draw_Menu_Item(row, ICON_ProbeZSpeed, F("Fast Probe Feed"));
            ...
            case PROBE_Z_FEEDR_SLOW:
              if (draw) {
                Draw_Menu_Item(row, ICON_ProbeZSpeed, F("Slow Probe Feed"));
    ...
    case Info:
          ...
          case LEVELING_GET_MESH:
            if (draw)
              ...
              gcode.home_all_axes(true);
                ...
                #if HAS_BED_PROBE
                  #if JYENHANCED
                    temp_val.cancel_lev = 0;
                    Confirm_Handler(Level);
                  #else
                    Popup_Handler(Level);
                  #endif
      ...
      case LevelSettings:
        ...
        #define LEVELING_SETTINGS_MESHINSET (LEVELING_SETTINGS_BEDTEMP + ENABLED(JYENHANCED))
          ...
          switch (item) {
            ...
            case MESHINSET_MINX:
              if (draw) {
                ...
                Draw_Float(eeprom_settings.mesh_min_x, row, false, 1);
              }
              else {
                temp_val.last_meshinset = eeprom_settings.mesh_min_x;
                Modify_Value(eeprom_settings.mesh_min_x, MIN_MESH_INSET, MAX_MESH_INSET - MIN_MESH_INSET, 1);
              }
              break;
            case MESHINSET_MAXX:
              if (draw) {
                Draw_Menu_Item(row, ICON_ProbeMargin, GET_TEXT_F(MSG_MESH_MAX_X));
                Draw_Float(eeprom_settings.mesh_max_x, row, false, 1);
              }
              else {
                temp_val.last_meshinset = eeprom_settings.mesh_max_x;
                Modify_Value(eeprom_settings.mesh_max_x, MIN_MESH_INSET , MAX_MESH_INSET - MIN_MESH_INSET, 1);
              }
              break;
            case MESHINSET_MINY:
              if (draw) {
                Draw_Menu_Item(row, ICON_ProbeMargin, GET_TEXT_F(MSG_MESH_MIN_Y));
                Draw_Float(eeprom_settings.mesh_min_y, row, false, 1);
              }
              else {
                temp_val.last_meshinset = eeprom_settings.mesh_min_y;
                Modify_Value(eeprom_settings.mesh_min_y, MIN_MESH_INSET, MAX_MESH_INSET - MIN_MESH_INSET, 1);
              }
              break;
            case MESHINSET_MAXY:
              if (draw) {
                Draw_Menu_Item(row, ICON_ProbeMargin, GET_TEXT_F(MSG_MESH_MAX_Y));
                Draw_Float(eeprom_settings.mesh_max_y, row, false, 1);
              }
              else {
                temp_val.last_meshinset = eeprom_settings.mesh_max_y;
                Modify_Value(eeprom_settings.mesh_max_y, MIN_MESH_INSET , MAX_MESH_INSET - MIN_MESH_INSET, 1);
              }
              break;
...
FSTR_P CrealityDWINClass::Get_Menu_Title(uint8_t menu) {
  switch (menu) {
    ...
    case InfoMain:          return GET_TEXT_F(MSG_INFO_SCREEN);
      ...
      #if JYENHANCED
        case MeshInsetMenu: return F("Mesh Inset Settings");
      #endif
  ...
  if (encoder_diffState == ENCODER_DIFF_CW) {
    ...
    switch (active_menu) {
      case Move:
        ...
        #if JYENHANCED
          case MeshInsetMenu:
            if ((temp_val.last_meshinset != (tempvalue / valueunit)) && (valuepointer != &eeprom_settings.probing_margin)) { temp_val.last_meshinset = 0; ApplyMeshLimits(); }
            break;
        #endif
      #endif
    }
    ...
    #if JYENHANCED
      if (active_menu == PhySetMenu) {
        JYEnhanced.ApplyPhySet();
        #if HAS_MESH
          MaxMeshArea();
        #endif
        #if ENABLED(NOZZLE_PARK_FEATURE)
          if (valuepointer == &eeprom_settings.x_max_pos) eeprom_settings.Park_point.x = eeprom_settings.Park_point.x + (eeprom_settings.x_max_pos - temp_val.last_pos); 
          if (valuepointer == &eeprom_settings.y_max_pos) eeprom_settings.Park_point.y = eeprom_settings.Park_point.y + (eeprom_settings.y_max_pos - temp_val.last_pos);
        #endif
        if (temp_val.axis == NO_AXIS_ENUM) {
        sprintf_P(cmd, PSTR("Warning! set X/Y max position according to the new bed size: %ix%i"), eeprom_settings.x_bed_size, eeprom_settings.y_bed_size);
        Update_Status(cmd);
        }
      }
    #endif
...
void CrealityDWINClass::Confirm_Control() {
  ...
  if (encoder_diffState == ENCODER_DIFF_ENTER) {
    switch (popup) {
      #if JYENHANCED && HAS_MESH
        case Level:
        temp_val.cancel_lev = 1;
        wait_for_user = false;
        Update_Status(F("Leveling cancelled"));
        Redraw_Menu(true, true, false);
        queue.inject(F("M84"));
        break;
      #endif
...
void CrealityDWINClass::Reset_Settings() {
  ...
  TERN_(FWRETRACT, temp_val.auto_fw_retract = fwretract.autoretract_enabled);
    ...
    eeprom_settings.x_min_pos  = DEF_X_MIN_POS;
    eeprom_settings.y_min_pos  = DEF_Y_MIN_POS;
    eeprom_settings.x_max_pos  = DEF_X_MAX_POS;
    eeprom_settings.y_max_pos  = DEF_Y_MAX_POS;
    eeprom_settings.z_max_pos  = DEF_Z_MAX_POS;
    #if HAS_MESH
      eeprom_settings.mesh_min_x = DEF_MESH_MIN_X;
      eeprom_settings.mesh_max_x = DEF_MESH_MAX_X;
      eeprom_settings.mesh_min_y = DEF_MESH_MIN_Y;
      eeprom_settings.mesh_max_y = DEF_MESH_MAX_Y;
    #endif
    #if HAS_BED_PROBE
...
//=============================================================================
    ...
    void CrealityDWINClass::MaxMeshArea() {
      #if HAS_BED_PROBE
        eeprom_settings.mesh_min_x = _MAX(PROBING_MARGIN, probe.offset.x);
        eeprom_settings.mesh_max_x = _MIN(X_BED_SIZE - PROBING_MARGIN, X_MAX_POS + probe.offset.x);
        eeprom_settings.mesh_min_y = _MAX(PROBING_MARGIN, probe.offset.y);
        eeprom_settings.mesh_max_y = _MIN(Y_BED_SIZE - PROBING_MARGIN, Y_MAX_POS + probe.offset.y);
      #else
        eeprom_settings.mesh_min_x = PROBING_MARGIN;  // PROBING_MARGIN_LEFT;
        eeprom_settings.mesh_max_x = X_BED_SIZE - PROBING_MARGIN;  // PROBING_MARGIN_RIGHT;
        eeprom_settings.mesh_min_y = PROBING_MARGIN;  // PROBING_MARGIN_FRONT;
        eeprom_settings.mesh_max_y = Y_BED_SIZE - PROBING_MARGIN;  // PROBING_MARGIN_BACK;
      #endif
      ApplyMeshLimits();
      Redraw_Menu(false);
    }
    ...
    void CrealityDWINClass::CenterMeshArea() {
      float max = eeprom_settings.mesh_min_x;
      if (max < X_BED_SIZE - eeprom_settings.mesh_max_x) max = X_BED_SIZE - eeprom_settings.mesh_max_x;
      if (max < eeprom_settings.mesh_min_y) max = eeprom_settings.mesh_min_y;
      if (max < Y_BED_SIZE - eeprom_settings.mesh_max_y) max = Y_BED_SIZE - eeprom_settings.mesh_max_y;
      eeprom_settings.mesh_min_x = max;
      eeprom_settings.mesh_max_x = X_BED_SIZE - max;
      eeprom_settings.mesh_min_y = max;
      eeprom_settings.mesh_max_y = Y_BED_SIZE - max;
      ApplyMeshLimits();
      Redraw_Menu(false);
    }
